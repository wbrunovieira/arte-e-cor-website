---
name: track-work
description: Rastreia todo trabalho do Arte e Cor Tintas website (features, correções, melhorias, débito técnico) como issues e milestones no WB Project Manager via a API, e mantém o status atualizado. Use SEMPRE ao planejar/iniciar trabalho não-trivial, ao descobrir um bug/melhoria, ao começar (→ In Progress) e ao concluir (→ Done). Também ao pedir "cria as issues", "cria o milestone", "atualiza o board".
---

# track-work — rastrear o Arte e Cor Tintas website como issues e milestones

Todo trabalho não-trivial vira issue no projeto **Arte e Cor Tintas website** do
WB Project Manager, agrupado em milestones, com status em dia. Não rastreie
...
# Issues
$PM find catalogo               # SEMPRE antes de criar — evita duplicata
...
- projectId: `cmu5eto8100dvp301rk65u3y9`  |  workspaceId: `cmge96f200001wa7ouziczg0w`
...
curl -s -X POST "$BASE/api/issues/bulk" -H "Authorization: Bearer $KEY" -H "Content-Type: application/json" \
  -d '{"workspaceId":"cmge96f200001wa7ouziczg0w","issues":[
        {"title":"...","description":"...","projectId":"cmu5eto8100dvp301rk65u3y9","statusId":"cmge9i3pv0007walqv7is970v","type":"FEATURE","priority":"HIGH"}
      ]}'
```

Doc completa das rotas: `/api/docs` (Swagger) · spec OpenAPI em `GET /api/openapi`.
...
- Obrigatórios ao criar issue: `title`, `workspaceId`, `statusId`. No POST single (`/api/issues`)
  o `workspaceId` vai no CORPO (não só no bulk) — sem ele = 400 Invalid input.
- Milestone exige `name` + `projectId`; `targetDate` é opcional e vai em ISO 8601.
- A issue volta só com `milestoneId` (sem objeto `milestone` aninhado); o `pm.sh` busca os nomes
  em `/api/milestones`. O bulk aceita `milestoneId` em cada issue.
