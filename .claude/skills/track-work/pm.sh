#!/usr/bin/env bash
# pm.sh — thin CLI for the WB Project Manager board (Arte e Cor Tintas website).
# Session tooling (gitignored under .claude/), so we stop hand-writing curl+python each time.
#
...
#   pm.sh create "Home — vitrine de tintas" --status todo --priority HIGH --milestone cmxxx
#   pm.sh ms set cmyyy cmxxx
set -euo pipefail

BASE="https://projects.wbdigitalsolutions.com"
PROJECT_ID="cmu5eto8100dvp301rk65u3y9"
...
# the issue API returns only milestoneId (no nested milestone object); map id -> name as JSON
_ms_map() {
...
for it in items:
    ...
    msn=(it.get('milestone') or {}).get('name') or MS.get(it.get('milestoneId'))
    tag=f\" [{msn}]\" if msn else ''
...
st=it.get('status',{}); stn=st.get('name') if isinstance(st,dict) else st
mid=it.get('milestoneId')
msn=(it.get('milestone') or {}).get('name') or MS.get(mid, mid)
print(it.get('id'),'|',stn,'|',it.get('title'))
print('milestone:', msn or '(none)')
print('---')
